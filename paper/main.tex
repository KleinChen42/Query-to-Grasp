...
\begin{table*}[t]
\centering
\caption{Final target-source ablation. PickCube uses fused memory grasp points;
StackCube guarded rows are pick-only; query-pick/oracle-place rows use
query-derived cubeA targets and a privileged cubeB destination; oracle
pick-place is fully privileged.}
\label{tab:target-source}
\begin{tabular}{llrrrrrl}
\toprule
Benchmark & Environment & Runs & Failed & Pick & Place & Task & Effective target source \\
\midrule
Full multi-view/closed-loop & \texttt{PickCube-v1} & 55 & 0 & 1.0000 & n/a & 0.1455 & \texttt{memory\_grasp\_world\_xyz} \\
Expanded tabletop & \texttt{StackCube-v1} & 50 & 0 & 0.6200 & n/a & 0.0000 & \texttt{task\_guard\_selected\_object\_world\_xyz} \\
Expanded closed-loop & \texttt{StackCube-v1} & 50 & 0 & 0.5200 & n/a & 0.0000 & \texttt{task\_guard\_selected\_object\_world\_xyz} \\
Query-pick/oracle-place single-view & \texttt{StackCube-v1} & 50 & 0 & 0.8800 & 0.7200 & 0.7200 & query target\,$\rightarrow$\,\texttt{oracle\_cubeB\_pose} \\
Query-pick/oracle-place tabletop & \texttt{StackCube-v1} & 50 & 0 & 0.6200 & 0.5200 & 0.5200 & query target\,$\rightarrow$\,\texttt{oracle\_cubeB\_pose} \\
Query-pick/oracle-place closed-loop & \texttt{StackCube-v1} & 50 & 0 & 0.5200 & 0.4800 & 0.4800 & query target\,$\rightarrow$\,\texttt{oracle\_cubeB\_pose} \\
Oracle pick & \texttt{PickCube-v1} & 50 & 0 & 1.0000 & n/a & 0.0400 & \texttt{oracle\_object\_pose} \\
Oracle pick & \texttt{StackCube-v1} & 50 & 0 & 0.9400 & n/a & 0.0000 & \texttt{oracle\_object\_pose} \\
Oracle pick-place & \texttt{StackCube-v1} & 50 & 0 & 0.9400 & 0.8800 & 0.8800 & \texttt{oracle\_cubeA\_pose}\,$\rightarrow$\,\texttt{oracle\_cubeB\_pose} \\
\bottomrule
\end{tabular}
\end{table*}
...
The paper pack, frozen main table, and supplemental-video manifest are generated
under \texttt{outputs/paper\_figure\_pack\_latest},
\texttt{outputs/paper\_submission\_audit\_latest}, and
\texttt{outputs/supplemental\_video\_latest}. Detailed commands are maintained
in the project README and paper-source README. The supplemental video uses six
representative ManiSkill execution clips captured from native \texttt{720x720}
RGB frames and letterboxed to \texttt{1920x1080}; these clips are presentation
artifacts, not additional benchmark rows.
