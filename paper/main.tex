...
success. Accepted query-driven StackCube rows remain pick-only until the
query-pick/oracle-place bridge is validated. These results
...
upper bound, and the current bridge tests whether query-derived cubeA targets
can support placement when cubeB is oracle-provided.
...
picking, oracle rows test privileged cubeA-to-cubeB placement, and the current
bridge tests query-derived cubeA targets with an oracle cubeB placement target.
...
oracle target-source comparison, and the pending query-driven StackCube
placement bridge.

\begin{table*}[t]
\centering
\caption{Target-source and simulated manipulation results used by the current
paper draft. Query-driven StackCube rows are pick-only until the placement
bridge is accepted; oracle pick-place is privileged.}
...
query-pick/oracle-place bridge is validated. Oracle pick-place rows are
privileged upper bounds, not deployable language-conditioned stacking.
...
\begin{itemize}
  ...
  \item \texttt{outputs/h200\_60071\_query\_stackcube\_place\_bridge\_seed0\_49} (pending bridge gate)
