...
\title{Query-to-Grasp: Language-Queryable RGB-D Retrieval, Multi-View Memory, and Simulated Pick Execution in ManiSkill}
...
Query-to-Grasp, a modular ManiSkill prototype for language-queryable RGB-D
target retrieval, multi-view object memory, confidence-aware re-observation
diagnostics, and opt-in simulated pick execution. The system connects
...
pick controller. The default executor remains a safe placeholder, while the
simulated executor reports downstream \texttt{pick\_success} separately from
task-level \texttt{task\_success}. On \texttt{PickCube-v1}, fused memory grasp
...
\texttt{0.9400}. StackCube task success remains \texttt{0.0000}, so we present
it as pick-only compatibility rather than stack completion. These results
support Query-to-Grasp as a reproducible diagnostic baseline for studying the
gap between open-vocabulary RGB-D retrieval and executable simulated grasp
targets.
\end{abstract}
...
pick-only compatibility, but task-aware guards, oracle baselines, and failure
reports show that remaining errors are target-source and association
limitations rather than evidence of full stacking competence.
...
\subsection{Simulated Pick Execution}
...
It reports \texttt{grasp\_attempted}, \texttt{pick\_success},
\texttt{task\_success}, pick stage, final TCP position, and simulator info.
\texttt{pick\_success} measures task-specific grasp or lift evidence, while
\texttt{task\_success} is the raw ManiSkill task success flag.
...
Experiments run in ManiSkill with RGB-D observations. The primary benchmark is
\texttt{PickCube-v1}. \texttt{StackCube-v1} is used as a cross-task pick-only
compatibility diagnostic: the system attempts to pick the queried red cube but
does not execute the placement phase required for stacking task completion.
...
\texttt{task\_success\_rate}, and pick-stage counts.
...
reports, full PickCube grasp comparison, StackCube expanded failure report, and
oracle target-source comparison.

\begin{table*}[t]
\centering
\caption{Target-source and simulated pick results used by the current paper
draft. StackCube rows are pick-only compatibility results; their
\texttt{task\_success\_rate} remains \texttt{0.0000}.}
...
\texttt{sim\_topdown} executor is a scripted top-down pick policy and does not
perform StackCube placement. StackCube \texttt{task\_success\_rate} remains
\texttt{0.0000}; its results are therefore pick-only compatibility evidence.
...
memory, uncertainty-aware re-observation, and opt-in simulated pick execution.
...
pick-only transfer while exposing target-source and closed-loop association
limits. These results provide a reproducible basis for a full simulated
conference-style paper and a clear roadmap toward broader task success.
