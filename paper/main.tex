...
Query-to-Grasp, a modular high-fidelity ManiSkill prototype for
...
summarize the final target-source results.
...
\caption{Frozen target-source comparison for the paper. PickCube demonstrates
that fused memory grasp points can be executed reliably, while StackCube
separates pick-only transfer, privileged oracle pick-place, and the
query-pick/oracle-place bridge.}
...
\caption{Expanded StackCube failure taxonomy. Closed-loop re-observation
introduces additional association failures even when uncertainty diagnostics
improve, so lower ambiguity does not automatically imply higher manipulation
success.}
...
The native-resolution supplemental demo captures are generated with:
\begin{verbatim}
...
The conference supplemental video storyboard and MP4 are assembled with:
\begin{verbatim}
...
The six supplemental clips are presentation artifacts. Their execution-video
metadata records native ManiSkill RGB source frames of \texttt{720x720} and a
letterboxed \texttt{1920x1080} output video.

The final submission-readiness audit and frozen main table are generated with:
\begin{verbatim}
python scripts/audit_paper_submission_package.py \
  --output-dir outputs/paper_submission_audit_latest
\end{verbatim}
...
\begin{itemize}
  ...
  \item \texttt{outputs/h200\_60071\_demo\_execution\_capture\_native720\_latest}
  \item \texttt{outputs/demo\_video\_pack\_latest}
  \item \texttt{outputs/supplemental\_video\_latest}
  \item \texttt{outputs/paper\_submission\_audit\_latest}
