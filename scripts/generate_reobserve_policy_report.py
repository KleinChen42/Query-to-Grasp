...
def benchmark_report(label: str, benchmark_dir: Path, max_examples: int) -> dict[str, Any]:
    ...
    return {
        ...
        "closed_loop_before_selected_still_selected_rate": _optional_float(
            metrics.get("closed_loop_before_selected_still_selected_rate")
        ),
        "closed_loop_before_selected_received_observation_rate": _optional_float(
        ...
        "mean_closed_loop_preferred_merge_rate": _optional_float(
            metrics.get("mean_closed_loop_preferred_merge_rate")
        ),
        "closed_loop_post_selection_continuity_apply_rate": _optional_float(
        ...
        "mean_closed_loop_before_selected_delta_num_observations": _optional_float(
            metrics.get("mean_closed_loop_before_selected_delta_num_observations")
        ),
...
def build_conclusion(benchmark_reports: list[dict[str, Any]]) -> str:
    ...
    if closed_loop_reports:
        ...
        preferred_merge_rate = _mean(
            _as_float(report.get("mean_closed_loop_preferred_merge_rate"))
            for report in closed_loop_reports
        )
        continuity_apply_rate = _mean(
            _as_float(report.get("closed_loop_post_selection_continuity_apply_rate"))
            for report in closed_loop_reports
        )
        ...
        if final_rate < initial_rate:
            return (
                ...
                f"(execution rate {execution_rate:.4f}, resolution rate {resolution_rate:.4f}, selected-object "
                f"association rate {selected_assoc_rate:.4f}, support-gain rate {selected_support_gain_rate:.4f})."
            )
        if final_rate == initial_rate:
            if selected_assoc_rate < execution_rate:
                ...
                f"preferred-merge rate was {preferred_merge_rate:.4f}, selection-continuity apply rate was {continuity_apply_rate:.4f}, "
            ...
            return (
                ...
                f"third-object involvement {third_object_rate:.4f}, preferred-merge rate {preferred_merge_rate:.4f}, "
                f"selection-continuity apply rate {continuity_apply_rate:.4f}, "
                f"support-gain rate {selected_support_gain_rate:.4f}). "
        ...
        return (
            ...
            f"resolution rate {resolution_rate:.4f}, selected-object association rate {selected_assoc_rate:.4f}). "
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    _format_optional_float(row["mean_closed_loop_preferred_merge_rate"]),
                    _format_optional_float(row["closed_loop_post_selection_continuity_apply_rate"]),
