...
def test_single_view_pick_benchmark_forwards_predicted_place_options(monkeypatch, tmp_path) -> None:
    seen_commands = []
    ...
    def fake_run(command, cwd, capture_output, text, check):
        ...
        summary = {
            ...
            "place_attempted": True,
            "place_success": True,
            "place_target_xyz": [0.1, 0.0, 0.05],
            "place_target_source": "predicted_place_object",
            "place_query": "cube",
            "place_selection_reason": "first_ranked_candidate_far_from_pick",
            "place_pick_xy_distance": 0.1,
            "task_success": True,
            "pick_stage": "success",
            "runtime_seconds": 1.5,
            "artifacts": str(run_dir),
    ...
    assert command[command.index("--place-min-distance-from-pick") + 1] == "0.05"
    ...
    assert rows[0]["place_query"] == "cube"
    assert rows[0]["place_selection_reason"] == "first_ranked_candidate_far_from_pick"
    summary = json.loads((output_dir / "benchmark_summary.json").read_text(encoding="utf-8"))
    assert summary["place_target_source"] == "predicted_place_object"
    assert summary["place_query"] == "cube"
